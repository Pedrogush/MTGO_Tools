"""Mana-symbol-aware RichTextCtrl (v2 rewrite, overlay-based hint).
...
Key design shift from v1: the placeholder hint is a separate
`wx.StaticText` widget positioned over the rich-text area rather than
text written into the buffer. v1 kept tripping over the fact that
applying a grey style for the hint leaves residue that later
contaminates typed characters; v2 keeps the buffer's style pristine so
typed text always renders in the single persistent dark style.
...
class ManaSymbolRichCtrl(wx.richtext.RichTextCtrl):
    ...
    def _sync_hint_visibility(self) -> None:
        show = (
            bool(self._hint_label.GetLabel())
            and not self._has_content()
