...
Which presses originate a marquee is decided live by an app-level event filter
(see ``main.py``) rather than by binding a handler onto every panel. The filter
intercepts every left-button press and asks :func:`is_background_window` whether
it landed on a plain non-interactive surface; if so it starts the active view's
marquee, otherwise it leaves the press to the widget under it (a button, or a
card/row inside a card view — which selects, or in the pile view begins a
drag-to-reorder). The decision is made at press time because the pile's
drag-to-reorder needs to own a press that lands on one of its cards; a press on
empty space — anywhere in the window — belongs to the marquee.
"""
...
# Plain background/decoration window types that the app-level filter treats as
# non-interactive surfaces a marquee may be drawn from. Exact-type checks (not
# isinstance) so the card-view canvases and the panels that subclass wx.Panel
# are never swept in, and so real controls (wx.Button, wx.Choice, … — which all
# derive from wx.Control like wx.StaticText does) keep their own press handling.
# The card views are classified separately (see the filter): a press inside one
# is "background" only when it misses every card.
_BACKGROUND_TYPES = (wx.Panel, wx.StaticText, wx.StaticLine)


def is_background_window(window: wx.Window | None) -> bool:
    """Whether a press on ``window`` should originate a marquee.

    True for the vanilla panels and static chrome that make up the application
    background — the surfaces with no press handling of their own. False for
    real controls and for every custom widget (including the card views, which
    the filter hit-tests for an empty spot separately), so their own click /
    drag gestures are never stolen.
    """
    return type(window) in _BACKGROUND_TYPES
