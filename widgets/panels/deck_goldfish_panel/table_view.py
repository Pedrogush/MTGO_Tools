...
* **right-click a card on the table** -- take it back to hand. The undo for a
  misplaced click, and the reason a mis-click costs nothing.
...
class GoldfishTableView(wx.Panel):
    ...
    # ----- geometry -----
    def _regions(self) -> tuple[Box, Box]:
        size = self.GetClientSize()
        return split_regions(size.GetWidth(), size.GetHeight())

    def _hand_boxes(self, region: Box) -> list[Box]:
        return hand_boxes(len(self._table.hand), region)

    def _table_boxes(self, region: Box) -> list[Box]:
        return [table_box(c.x, c.y, c.tapped, region) for c in self._table.table]
    ...
    def _on_paint(self, _event: wx.PaintEvent) -> None:
        ...
        dc.Clear()
        table_region, hand_region = self._regions()
        ...
        for card, box in zip(self._table.table, self._table_boxes(table_region)):
            self._draw_card(dc, box, card.name, tapped=card.tapped)
        for name, box in zip(self._table.hand, self._hand_boxes(hand_region)):
            self._draw_card(dc, box, name)
        if self._dragging and self._drag_point is not None:
            self._draw_drag_ghost(dc)

    def _draw_card(self, dc: wx.DC, box: Box, name: str, *, tapped: bool = False) -> None:
        dc.DrawBitmap(self._art.bitmap(name, tapped=tapped), box.x, box.y, True)
        if tapped:
    ...
    def _draw_drag_ghost(self, dc: wx.DC) -> None:
        ...
        bitmap = self._art.bitmap(name, tapped=tapped)
        dc.DrawBitmap(
            bitmap,
    ...
    def _on_left_down(self, event: wx.MouseEvent) -> None:
        ...
        table_region, hand_region = self._regions()
        index = hit_test(self._hand_boxes(hand_region), point.x, point.y)
        if index is not None:
            self._press = (_FROM_HAND, index, point)
        else:
            index = hit_test(self._table_boxes(table_region), point.x, point.y)
            if index is None:
                return
    ...
    def _on_left_up(self, event: wx.MouseEvent) -> None:
        ...
        table_region, _hand_region = self._regions()
        if zone == _FROM_HAND:
            if not dragging:
                x, y = auto_place_fraction(self._auto_placed)
                self._auto_placed += 1
            elif table_region.contains(point.x, point.y):
                x, y = drop_fraction(point.x, point.y, table_region)
            else:
