...
and the metagame-analysis viewer need (``get_archetypes_for_format`` and
``get_stats_for_format``), routing both through a single :class:`MetagameService`
instance backed by the default repository.
"""
...
class MetagameService:
    ...
    def get_archetypes_for_format(self, mtg_format: str) -> list[dict[str, Any]]:
        """Return the archetype list for ``mtg_format`` via the underlying repository."""
        return self.metagame_repo.get_archetypes_for_format(mtg_format)
