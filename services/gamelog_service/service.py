...
from services.gamelog_service.archetypes import ArchetypeClassifierProto, detect_archetype
from services.gamelog_service.discovery import find_all_gamelog_dirs, find_gamelog_files
from services.gamelog_service.formats import RarityIndexProto, detect_format_from_cards
...
def parse_gamelog_file(
    ...
    archetype_model: ArchetypeClassifierProto | None = None,
) -> dict | None:
    try:
        ...
        # Each player's cards are classified on their own, scoped to the
        # pooled format: a known format narrows the candidates to its own
        # archetypes, "Unknown" lets every format's archetypes compete.
        player1_archetype = detect_archetype(player1_deck, archetype_model, detected_format)
        player2_archetype = detect_archetype(player2_deck, archetype_model, detected_format)
...
def parse_all_gamelogs(
    ...
    for i, file_path in enumerate(log_files):
        ...
        match_data = parse_gamelog_file(file_path, card_manager, rarity_index, archetype_model)
        if match_data:
